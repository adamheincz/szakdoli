...
public class NetworkRoomPlayerOwn : NetworkRoomPlayer
{
    [Header("UI")]
    [SerializeField]
    private TMP_Text[] playerNameTexts = new TMP_Text[5];
    [SerializeField]
    private TMP_Text[] playerReadyTexts = new TMP_Text[5];
    [SerializeField]
    ...
    {
        Debug.Log("on_client_enter_room");
    }
    ...
    public void DisplayNameChanged(string oldName, string newName) { }// => UpdateDisplay();
    ...
    public override void OnGUI()
    {
        ...
        playerReadyTexts[index].text = readyToBegin ? "<color=#42bc7f>READY</color>" : "<color=#DD3745>NOT READY</color>";
        playerNameTexts[index].transform.parent.gameObject.SetActive(true);
        //Debug.Log("gameobject_name: " + playerReadyTexts[index].transform.parent.gameObject.name);
        //Debug.Log("playerNameTexts[index].text = Player + (index + 1): " + "Player" + (index + 1));
        ...
        if(NetworkClient.active && isLocalPlayer)
        {
            ...
            if (readyToBegin)
            {
                readyButton.GetComponentInChildren<TextMeshProUGUI>().text = "<color=#42bc7f>READY</color>";
                readyButton.onClick.AddListener(() => CmdChangeReadyState(false));
            }
            else
            {
                readyButton.GetComponentInChildren<TextMeshProUGUI>().text = "<color=#DD3745>NOT READY</color>";
                readyButton.onClick.AddListener(() => CmdChangeReadyState(true));
            }
        } else
        {
